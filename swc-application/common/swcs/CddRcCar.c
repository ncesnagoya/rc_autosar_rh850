 ...
 *  の責任を負わない．
 *
 *  $Id: CddRcCar.c 122 2015-07-17 01:31:28Z ertl-honda $
 */  

/*
 *		CDD SW-C CddRcCar用の記述
 */

#include "Rte_CddRcCar.h"
 ...
 * ステアリング値/車速/ブレーキの指令値
 */
static volatile IDT_Angle	angle_val = 0;
static volatile IDT_Speed	speed_val = 0;
static volatile IDT_Brake	brake_val = FALSE;
...
/*
 * ステアリング値設定用RunnableEntity(オペレーション呼出し)
 */
void
SetSteerAngle(IDT_Angle angle)
{
...
/*
 * 車速値設定用RunnableEntity(オペレーション呼出し)
 */
void
SetDriveSpeed(IDT_Speed speed)
{
...
/*
 * ブレーキ値設定用RunnableEntity(オペレーション呼出し)
 */
void
SetBrake(IDT_Brake brake)
{
	brake_val = brake;
}
...
PowerTrain(void)
{
	...
	static IDT_Brake	pre_brake_val = FALSE;
...
void
VehicleInfoReporter(void)
{
	/* 操舵角送信 */
	Rte_Write_VehicleInfoOut_angle(angle_val);

	/* 車速送信 */
	Rte_Write_VehicleInfoOut_speed(speed_val);

	/* ステアリングと車速のPWM設定値をマイクロ秒単位で送信 */
	Rte_Write_VehicleInfoOut_pwm_angle((IDT_PWM) servo_pwm);
	Rte_Write_VehicleInfoOut_pwm_speed((IDT_PWM) (esc_pwm_100nsec / 10));
}
